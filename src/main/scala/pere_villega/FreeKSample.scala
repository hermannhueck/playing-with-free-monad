...
object FreeKSample extends App {
  ...
  object Program {
    ...
    val programWithList: Free[PRG.Cop, Response] = {
      import cats.instances.list._
      import cats.syntax.traverse._
      for {
        st <- ListStocks().freek[PRG]
        _ <- st.traverse(Buy(_, 100).freek[PRG])
    ...
    val interpreter: Interpreter[PRG.Cop, Id] = LogInterpreter :&: AuditInterpreter :&: OrderInterpreter
    ...
    val interpreterWithMessaging: Interpreter[PRG.Cop, Id] =
